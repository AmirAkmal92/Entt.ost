...
objectbuilders.app, 'partial/consigment-request-details-bulk'],
...
function(context, logger, router, system, validation, eximp, dialog, watcher, config, app, partial) {
    ...
    var entity = ko.observable(new bespoke.Ost_consigmentRequest.domain.ConsigmentRequest(system.guid())),
        ...
        activate = function(entityId) {
            ...
            context.loadOneAsync("EntityForm", "Route eq 'consigment-request-details-bulk'")
                .then(function(f) {
                form(f);
                ...
                .then(function(w) {
                watching(w);
                return $.getJSON("i18n/" + config.lang + "/consigment-request-details-bulk");
            })
                .then(function(n) {
                i18n = n[0];
        ...
        attached = function(view) {
            // validation
            validation.init($('#consigment-request-details-bulk-form'), form());
        ...
        saveCommand = function() {
            return defaultCommand()
                .then(function(result) {
                if (result.success) {
                    return app.showMessage("Sender Information Saved.", ["OK"]);
                } else {
                    return Task.fromResult(false);
                }
            })
                .then(function(result) {
                if (result) {
                    router.navigate('consignment-request-receiver-bulk/' + entity().Id());
                }
            });
        };
    var vm = {
